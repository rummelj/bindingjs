...
A *Scope* consists of one or more CSS selectors separated by commas and and a *Scope Body* that is enclosed in curly braces. The Scope Body may contain *Bindings* or other Scopes, which can be seen as a Tree of Scopes. Each Scope slices a portion out of the template, which can be refined when nesting deeper. The idea is, that any Binding applies to all elements that are matched by its enclosing Scope. In the example the first and last Binding would apply to any elements of the template which have *wrapper* as their id. Assuming that there is exactly one such wrapper element, the second Binding would then apply to all of its descendants which are either a *span* or have the class *input* a *div* as their parent. Here is the same example without nesting. Obviously more code has to be repeated now.
...
There is one speciality about the binding scope adapter. If it is used within a scope, it becomes visible to all descendants of that scope. This means, that the same qualifier for the binding scope might not necessarily refer to the same value.
...
One use case for the binding scope adapter is to realize dependant view elements. Imaginge the situation, where you want to only enable a submit button, if the value of a text box is not empty. 
...
| Relational     | `Expr == Expr`<br>`Expr === Expr`<br>`Expr != Expr`<br>`Expr !== Expr`<br>`Expr <= Expr`<br>`Expr >= Expr`<br>`Expr < Expr`<br>`Expr > Expr`<br> | `$name === "admin"`<br>`$duration != 0`<br>`$age <= 120`<br>`$end >= $start`<br>`!($money < $cost)`<br>`$amount > 0`|
...
Identification allows to name and later identify certain parts of a binding specification. Syntactically it can placed, wherever a scope could be placed. One of its purposes is to only use a certain part of the binding specification. Please refer to the [API documentation](https://github.com/rummelj/bindingjs/wiki/Application-User-Interface-(API)) for more information.

###Insertion

Insertion allows to mark certain parts of the template as hooks for external content. A possible use case would be, that you want to initialize a third-party library for each item that is created when iterating a collection.
